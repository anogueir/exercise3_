...
garden_harvest %>% 
  ...
  group_by(vegetable,weekday) %>% 
    summarise(tot_weight = sum(wt_lbs)) %>% 
...
garden_harvest %>% 
  ...
  group_by(variety2) %>% 
  summarise(wt_lbs = sum(weight)*0.00220462) %>%
ggplot(aes(y= variety2, x= wt_lbs)) +
  geom_col()
...
garden_harvest %>% 
  ...
  distinct(variety_lower, vegetable, variety_length)
...
Stations_trips <- Trips %>% 
  left_join(Stations,
            by= c("sstation" = "name")) %>% 
  group_by(lat, long) %>% 
  summarise(total_departure = n()) 


  ggplot (Station_trips, aes(x= lat, y= long, color = total_departure)) +
  geom_point() +
...
```{r}
summarise(total_departure = n(), `Proportion of Casual Trips` = mean(client == "Casual"))
```
